...
//errMapBean = com.ktds.targetatom.bean.ErrMappgBasBean;
...
if(!CollectionUtils.isEmpty(bodyList)) {
 ...
 log.info("file total_count ::: " + total_count);
 exchange.getIn().setHeader("file.total.count", total_count);
 
 log.info("file success_count ::: " + success_count);
 exchange.getIn().setHeader("file.success.count", success_count);
 
 log.info("file error_count ::: " + error_count);
...
// comnStrTrim(uBody.application_release_indicator);
    if (uBody.application_release_indicator == "") {
 ...
 if (uBody.service_charging_type == "004" || uBody.service_charging_type == "006") {
 ...
 if ((uBody.bIsVOLTE_4G_CALL) && (uBody.hd_call == "1") && 
 ...
 if (uBody.service_charging_type == "001"){
 ...
 if (uBody.supplementary_services_code_1 == "227") {
 ...
 // uBody.setsChrgSvcNo(enriched.getsCallingSvcNo());
  ...
  if (uBody.getLST_DADAN_CALL().contains(uBody.getWstype())) {
     ...
     if (uBody.bIsHD_4G_VOICE) {
         ...
         if (uBody.terminating_inter_operator_identifier == "ims.kt.c") {
  ...
  var sPreRoutingNumber = comnStrTrim(comnSubstring(uBody.routing_number, 0, 2));
     ...
     if (!(sPreRoutingNumber != "" || sPreRoutingNumber == null)) {
         uBody.sNpInd = "Y";
         uBody.sNpOperator = comnPadChr(sPreRoutingNumber, 2, "0", "L");
     }

     
        // String sPreRoutingNumber = "";
        
     // if (!( volteBody.getRouting_number().equals("") ||
		// volteBody.getRouting_number() == null )) {
     // sPreRoutingNumber = comnSubstring(volteBody.getRouting_number(), 0,
		// 2).trim();
     // enriched.setsNpInd("Y");
     // enriched.setsNpOperator(padString(sPreRoutingNumber, '0', 2, "L"));
     // }
     ...
     if (sPreRoutingNumber == "" || sPreRoutingNumber == null) {
         if ((uBody.supplementary_services_code_1 == "241") && (uBody.service_category_id == "611" || uBody.service_category_id == "000")) {
         ...
         if (uBody.terminating_inter_operator_identifier == "ims.kt.c") {
             ...
             uBody.sNpInd = "Y";
         } else if (uBody.terminating_inter_operator_identifier == "octave.c") {
...
//     
     ...
     if (uBody.sSpIndi == "Y" && uBody.bIsINTERNATIONAL) {
  ...
  //
     ...
     if (uBody.isbIsTIME_CDR() || uBody.isbIsTIME_CDR_001()) {
      uBody.setiDurSecPrepaid(Integer.parseInt(uBody.getService_prepaid_airtime_charge()) + Integer.parseInt(uBody.getService_prepaid_information_charge()));
  ...
  tbWlessSpclNoBas = audit.get_TB_WLESS_SPCL_NO_BAS(uBody.getsCalledArea(), uBody.getsChrgOtherNo(), "KTF", uBody.getsCallStartDateTime(), uBody.getSpnm());
     ...
     // }
      if ((uBody.isbIsNORMAL_COMP()) && (uBody.getsSpIndi().equals("Y"))) {
       uBody.setiSpecialNoId(Integer.parseInt(tbWlessSpclNoBas.getlSpecialNoId()));
      }
     ...
     // }
      if ((uBody.isbIsPSVT_3G_CALL() || uBody.isbIsPSVT_4G_CALL()) && !(uBody.isbIsWIN_CHARGED())) {
       ...
       iDeductTime = Integer.parseInt(uBody.getsFpsResult());
...
//	     uErrCdOut.iErrPriority    = "1000";     // MZ_ERR_PRIRT_NO
//	     uErrCdOut.sOldErrCd	   = "IMSI";
	...
	tbCalNoBas = audit.get_TB_CAL_NO_BAS (comnSubstring(uBody.sChrgSvcNo, 0, 3), comnSubstring(uBody.sChrgSvcNo, 3, 7), comnSubstring(uBody.sChrgSvcNo, 7, 11), uBody.sCallStartDate);
	...
	log.debug("Message Error uBOdy : {}", uBody.sErrCd ,uBody.sErrLevelDivCd , uBody.iErrPriority  ,uBody.sOldErrCd);
	if ( ltErrorCd != null ) {
