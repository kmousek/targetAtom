...
public class PRINTKResultCdr extends ConmonCdr {
	...
	@DataField(pos=2, length=2, trim=true, defaultValue = "04")
	String source_format;
 
	@DataField(pos=4, length=16, trim=true) //, align="L"
	String calling_number;
 
	@DataField(pos=20, length=16, trim=true) //, align="L"
	String called_number;
 
	@DataField(pos=36, length=16, trim=true)
	String sChargingNumber;
 
	@DataField(pos=52, length=3, trim=true)
	String swin_service_type;
 
	@DataField(pos=55, length=8, trim=true, pattern="yyyyMMdd")
	String call_start_date;
 
	@DataField(pos=63, length=6, trim=true, pattern="HHmmss")
	String call_start_time;
 
	@DataField(pos=69, length=8, trim=true, pattern="yyyyMMdd")
	String sEventEndDate;
	
	@DataField(pos=77, length=6, trim=true, pattern="HHmmss")
	String sEventEndTime;
 
	@DataField(pos=83, length=10, trim=true, pattern="HHmmss") //, align="L"
	String use_time;
 
	@DataField(pos=93, length=12, trim=true, align="R")
	int charge_amount;
 
	@DataField(pos=105, length=5, trim=true)
	String intl_prefix;
 
	@DataField(pos=110, length=8, trim=true, align="R") // sNationCode 필드 사이즈 증가 3 -> 8 ----- 2016.04.12
	String nation_code;
 
	@DataField(pos=118, length=1, trim=true)
	String sOriginOperator;
 
	@DataField(pos=119, length=1, trim=true)
	String sCallKind;
 
	@DataField(pos=120, length=16, trim=true)
	String sIMSI;
 
	@DataField(pos=136, length=3, trim=true)
	String sService_Feature_Type;
 
	@DataField(pos=139, length=15, trim=true)
	String sSGSNIPaddress;
 
	@DataField(pos=154, length=2, trim=true)
	String service_ind;
 
	@DataField(pos=156, length=5, trim=true) //, align="R"
	String call_code;

	@DataField(pos=161, length=5, trim=true) //, align="L"
	String dis_code;
	//BigDecimal service_prepaid_airtime_charge;

	@DataField(pos=166, length=10, trim=true) //, align="L"
	int discount_amount;
 
	@DataField(pos=176, length=2, trim=true)
	String sCallingsaup;

	@DataField(pos=178, length=2, trim=true)
	String sCalledsaup;

	@DataField(pos=180, length=10, trim=true, align="R")
	String sDirectConnectFee;

	@DataField(pos=190, length=3, trim=true)
	String sService_type;

	@DataField(pos=193, length=8, trim=true)
	String sTerminating_InterOperaor_ID;

	@DataField(pos=201, length=1, trim=true)
	String sHDcallindicator;
 
	@DataField(pos=202, length=4, trim=true)
	String sSupplimentaryservicecode;

	@DataField(pos=206, length=16, trim=true)
	String sServiceStart3GPPUserLocationInfo;

	@DataField(pos=222, length=3, trim=true)
	String sServiceCategoryID_VoLTE;

	@DataField(pos=225, length=4, trim=true)
	String sApplicationReleaseIndicator;

	@DataField(pos=229, length=3, trim=true)
	String sServiceChargingType;
 
	@DataField(pos=232, length=10, trim=true, align="R")
	String sServicePrepaidAirtimeCharge;

	@DataField(pos=242, length=10, trim=true, align="R")
	String sServicePrepaidCountCharge;
 
	@DataField(pos=252, length=10, trim=true, align="R")
	String sServicePrepaidInformationCharge;
 
	@DataField(pos=262, length=3, trim=true)
	String sCallType;

	@DataField(pos=265, length=10, trim=true, align="R")
	String sServiceContentsCount;

	@DataField(pos=275, length=11, trim=true)
	String sRoutingNumber;

	@DataField(pos=286, length=30, trim=true)
	String sServiceCalledInformation;
 
	@DataField(pos=316, length=6, trim=true)
	String sSgsnMccMnc;

	@DataField(pos=322, length=5, trim=true)
	String sMessageRate;
 
	@DataField(pos=327, length=5,trim=true)
	String sChargePerMessage;
 
	@DataField(pos=332, length=1, trim=true)
	String sGubun1;

	@DataField(pos=333, length=5, trim=true)
	String sTotalServiceRate;

	@DataField(pos=338, length=1, trim=true)
	String sSvcProvider;

	@DataField(pos=339, length=15, trim=true)
	String sCallingAssertedIdentity;
 
	@DataField(pos=354, length=1, trim=true)
	String sInNetCallYn;

	@DataField(pos=355, length=3, trim=true)
	String sCarrierCode;

	@DataField(pos=358, length=1, trim=true)
	String sNpIndicator;

	@DataField(pos=359, length=20, trim=true)
	String sReserved;

	@DataField(pos=379, length=1, trim=true, paddingChar=' ')
	String sLineFeed;

	public PRINTKResultCdr() {
		
	}
 
	public PRINTKResultCdr(PRINTKBody org) {
	...
	public String getsLineFeed() {
		return sLineFeed;
	}

	public void setsLineFeed(String sLineFeed) {
		this.sLineFeed = sLineFeed;
	}
	...
	public String toString() {
		return "PRINTKResultCdr [log=" + log + ", record_type=" + record_type + ", source_format=" + source_format
